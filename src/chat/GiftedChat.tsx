...
> {
  ...
  private _actionSheetRef: any;
  private _messageContainerRef: any;

  private textInput: any;

  //private invertibleScrollViewProps: any;
  ...
  constructor(props: GiftedChatProps) {
    ...
    this.onKeyboardDidShow =
      this.onKeyboardDidShow.bind(this);

    this.onKeyboardDidHide =
      this.onKeyboardDidHide.bind(this);

    this.onSend = this.onSend.bind(this);
    this.getLocale = this.getLocale.bind(this);

    this.onInputSizeChanged =
      this.onInputSizeChanged.bind(this);

    this.onInputTextChanged =
      this.onInputTextChanged.bind(this);

    this.onMainViewLayout =
      this.onMainViewLayout.bind(this);

    this.onInitialLayoutViewLayout =
      this.onInitialLayoutViewLayout.bind(this);
  ...
  componentDidUpdate(
    previousProps: GiftedChatProps,
  ): void {
    if (
      previousProps.messages !==
      this.props.messages
    ) {
      this.setMessages(
        this.props.messages ?? [],
    ...
    if (
      ...
      this.setTextFromProp(
        this.props.text,
      );
    }

    if (
      previousProps.locale !==
      this.props.locale
    ) {
      this.initLocale();
    }
  ...
  initLocale(): void {
    ...
    if (
      !locale ||
      !moment.locales().includes(locale)
    ) {
      this.setLocale('en');
      return;
    }
  ...
  setTextFromProp(text?: string): void {
    if (typeof text === 'string') {
      this.onInputTextChanged(text);
    }
  }

  getTextFromProp(fallback: any) {
  ...
  getMaxHeight() {
    return this._maxHeight as number;
  }

  setKeyboardHeight(height: any) {
  ...
  calculateInputToolbarHeight(composerHeight: any) {
   ...
   */
  getBasicMessagesContainerHeight(_composerHeight = this.state.composerHeight) {
  ...
  prepareMessagesContainerHeight(value: any) {
    ...
    return (
      ...
      >
        <MessageContainer
          ...
          ref={(component: any) => (this._messageContainerRef = component)}
  ...
  onInputSizeChanged(size: any) {
    const newComposerHeight = Math.max(MIN_COMPOSER_HEIGHT as number, Math.min(MAX_COMPOSER_HEIGHT, size.height));
    const newMessagesContainerHeight = this.getMessagesContainerHeightWithKeyboard(newComposerHeight);
    this.setState({
      composerHeight: newComposerHeight,
      messagesContainerHeight: this.prepareMessagesContainerHeight(newMessagesContainerHeight),
